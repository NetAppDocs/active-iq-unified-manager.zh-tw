...
+
.. 執行下列命令、將內容設定為「mysqld_db_t」：「emanage fcontext-A -t mysqld_db_t「/opp/netapp/data」（/opp/datape/data）」、「restorecon -R -v /opp/netapp/data」
...
+
.. 執行下列命令、將內容設定為「mysqld_db_t」：「emanage fcontext-A -t mysqld_db_t「/opt/netapp」（英文）「restorecon -R -v /opt/netapp」（英文）
