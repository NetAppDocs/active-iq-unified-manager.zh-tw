...
---
= 在 Red Hat Enterprise Linux 上安裝 Active IQ Unified Manager
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
|===
. 如果已安裝的 MySQL 版本低於 MySQL 8.4.10 Community Edition，請輸入以下命令將其解除安裝：
+
...
+
對於 Red Hat Enterprise Linux 8 系統，如果您有包含 JDK 17 - OpenJDK、7-zip 以及 AppStream 儲存庫提供的其他軟體包的內部儲存庫，則必須停用 AppStream 儲存庫並安裝 MySQL Community Server。運行以下命令：
