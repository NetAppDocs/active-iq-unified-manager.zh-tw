...
---
= 了解 Active IQ Unified Manager 的虛擬化基礎架構和硬體需求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 分配給根分割區的50 GB容量
* 100 GB可用磁碟空間、分配給「/opt/NetApp/data」目錄、該目錄安裝在LVM磁碟機或連接至目標系統的個別本機磁碟上


[NOTE]
====
對於單獨掛載的「/opt」和「/var/log」目錄、請確定「/opt」有15 GB的可用空間、而「/var/log」有16 GB的可用空間。「/tmp」目錄至少應有10 GB的可用空間。
...
此外，單一 Unified Manager 實例可監控的節點數量有限制，超過限制後，您需要安裝第二個 Unified Manager 實例。有關節點限制、CPU 和記憶體建議，請參閱 https://www.netapp.com/pdf.html?item=/media/13504-tr4621.pdf["Unified Manager最佳實務做法指南"^]。
